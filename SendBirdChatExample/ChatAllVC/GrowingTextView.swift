...
open class GrowingTextView: UITextView {
    ...
    private var heightConstrainttt: NSLayoutConstraint?
    ...
    override open func layoutSubviews() {
        ...
        if (heightConstrainttt == nil) {
            heightConstrainttt = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: height)
            addConstraint(heightConstrainttt!)
        }
        
        // Update height constraint if needed
        if height != heightConstrainttt!.constant {
            shouldScrollAfterHeightChanged = true
            heightConstrainttt!.constant = height

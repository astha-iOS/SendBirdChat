...
//        let image = FLAnimatedImage.init(animatedGIFData: NSData(contentsOfFile: Bundle.main.path(forResource: "loading_typing", ofType: "gif")!) as Data?)
  ...
  //      self.typingIndicatorContainerView.isHidden = true
        ...
        self.tblChat.estimatedRowHeight = 140.0
        self.tblChat.delegate = self
        self.tblChat.dataSource = self
        self.tblChat.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 14, right: 0)
        
        self.tblChat.register(GroupChannelIncomingUserMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelIncomingUserMessageTableViewCell")
        self.tblChat.register(GroupChannelIncomingImageVideoFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelIncomingImageFileMessageTableViewCell")
        self.tblChat.register(GroupChannelIncomingImageVideoFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelIncomingVideoFileMessageTableViewCell")
        self.tblChat.register(GroupChannelOutgoingUserMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelOutgoingUserMessageTableViewCell")
        self.tblChat.register(GroupChannelNeutralAdminMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelNeutralAdminMessageTableViewCell")
        self.tblChat.register(GroupChannelOutgoingImageVideoFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelOutgoingImageFileMessageTableViewCell")
        self.tblChat.register(GroupChannelOutgoingImageVideoFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelOutgoingVideoFileMessageTableViewCell")
        self.tblChat.register(GroupChannelOutgoingGeneralFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelOutgoingGeneralFileMessageTableViewCell")
        self.tblChat.register(GroupChannelIncomingGeneralFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelIncomingGeneralFileMessageTableViewCell")
        self.tblChat.register(GroupChannelOutgoingAudioFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelOutgoingAudioFileMessageTableViewCell")
        self.tblChat.register(GroupChannelIncomingAudioFileMessageTableViewCell.nib(), forCellReuseIdentifier: "GroupChannelIncomingAudioFileMessageTableViewCell")
        
        let messageViewTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(hideKeyboard(recognizer:)))
        self.tblChat.addGestureRecognizer(messageViewTapGestureRecognizer)
...
//        }
        ...
        //--------create a connection
               ...
               let myUserID:Int = UserDefaults.standard.value(forKey: "isMeUserId") as! Int
               let myUserName:String = UserDefaults.standard.value(forKey: "isMeUserName") as! String

               user_id_str = String(myUserID)
               print("-------Neha user_id string-----",myUserID)
               print("-------Neha name string-----",myUserName)

              SBDMain.connect(withUserId: user_id_str, completionHandler: { (user, error) in
                   guard error == nil else { // Error
...
//                })
        ...
        if comesFrom == "notification"{
            if channerlURL != "urlNotMatch"{
            print("-------------------------------------------->>>",channerlURL);
              
            SBDGroupChannel.getWithUrl(channerlURL) { (groupChannel, error) in
                
                guard error == nil else {   // Error.
                    print(error?.localizedDescription ?? "")
                      return
                  }
                print(groupChannel as Any)
                self.myChannel = groupChannel
                print(groupChannel?.name ?? "")
                self.loadPreviousMessages(initial: true)
               
                
            }
            }else{
                    let controller = UIAlertController(title: "Alert!", message: "channel not found", preferredStyle: .actionSheet)
        ...
        SBDGroupChannel.createChannel(withUserIds: [user_id_str,friendID], isDistinct: true, completionHandler: { (groupChannel, error) in
            guard error == nil else {   // Error.
                 kb.topMostVC?.stopAnimating()
                 print(error?.localizedDescription ?? "")
                return
        ...
        myChannel?.inviteUserIds([friendID], completionHandler: { (error) in
...
//
//        groupChannel?.inviteUserId(friendID, completionHandler: { (error) in
//
//                guard error == nil else { // Error
//                    print(error?.localizedDescription ?? "")
//                    return
...
//        self.navigationItem.setHidesBackButton(false, animated: true);
   ...
   //     self.imgView?.loadImage(strUrl: user.avatar?.medium)
       print("--imgggg--",friendImage)
        

                let containView = UIView(frame: CGRect(x: 0, y: 0, width: 40, height: 40))
                let imageview = UIImageView(frame: CGRect(x: 0, y: 0, width: 40, height: 40))
                imageview.image = UIImage(named: "profile_sel.jpg")
                imageview.contentMode = .scaleAspectFit
                imageview.loadImage(strUrl: friendImage)
                imageview.contentMode = UIView.ContentMode.scaleAspectFit
                imageview.layer.cornerRadius = 20
                imageview.layer.borderColor = UIColor(red: 48.0, green: 195.0, blue: 147, alpha: 1.0).cgColor
    ...
    @objc private func btnMessagesAction() {
       }
   ...
   //     self.navigationController?.setNavigationBarHidden(true, animated: false)
    ...
    func loadPreviousMessages(initial: Bool) {
       ...
       print("timeStepm = timestamp",timestamp)
        
        if self.hasPrevious == false { return }
        
        guard let channel = self.myChannel else {
...
//                    }
                    ...
                    if messages.count > 0 {
                        DispatchQueue.main.async {
                            ...
                            for message in messages {
                                self.messages.append(message)  /// first load
                                
                                print("------Astha-----",message)
                                print("------Yadav-----",self.messages)
                            ...
                            self.tblChat.scrollToRow(at: IndexPath(row: messages.count-1, section: 0), at: .top, animated: false)
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        ...
        //    self.bottomViewBottomConstraint.constant = keyboardSize.height-view.safeAreaInsets.bottom
            hg = Int(keyboardSize.height-view.safeAreaInsets.bottom)
        }else{
        //    self.bottomViewBottomConstraint.constant = keyboardSize.height
            hg = Int(keyboardSize.height)
        }
       //Utils.getKeyboardAnimationOptions(notification: notification)
     //   let (height, duration, _) =
        
        self.keyboardHeight = CGFloat(hg)
        
        DispatchQueue.main.async {
            UIView.animate(withDuration: 0.1 , delay: 0, options: .curveEaseOut, animations: {
                self.bottomViewBottomConstraint.constant = self.keyboardHeight - self.view.safeAreaInsets.bottom
                self.view.layoutIfNeeded()
            }, completion: nil)
    ...
    func scrollToBottom(force: Bool) {
        ...
        let currentRowNumber = self.tblChat.numberOfRows(inSection: 0)
        
        self.tblChat.scrollToRow(at: IndexPath(row: currentRowNumber - 1, section: 0), at: .bottom, animated: true)
    }
    ...
    func scrollTo(position: Int) {
        ...
        self.tblChat.scrollToRow(at: IndexPath(row: position, section: 0), at: .top, animated: false)
    }
    
    // MARK: - Keyboard
    func determineScrollLock() {
        if self.messages.count > 0 {
            if let indexPaths = self.tblChat.indexPathsForVisibleRows {
                if let lastVisibleCellIndexPath = indexPaths.last {
                    let lastVisibleRow = lastVisibleCellIndexPath.row
                    if lastVisibleRow < self.messages.count {
...
//        //MARK:-  keyboard hide show notification center
...
//        self.view.layoutIfNeeded()
//
//        print("keyboard height",keyboardSize.height,"safe area", view.safeAreaInsets.bottom)
//    }
//
...
//
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if currMessage is SBDAdminMessage {
            ...
            if sender.userId == SBDMain.getCurrentUser()!.userId {
                ...
                userMessageCell.channel = self.myChannel
            ...
            if let _ = self.preSendMessages[fileMessageRequestId] {
                ...
                if (fileDataDict["type"] as! String).hasPrefix("image") {
                    ...
                    imageFileMessageCell.channel = self.myChannel
                    ...
                    videoFileMessageCell.channel = self.myChannel
                    ...
                    audioFileMessageCell.channel = self.myChannel
                    ...
                    generalFileMessageCell.channel = self.myChannel
    ...
    func channel(_ sender: SBDBaseChannel, didReceive message: SBDBaseMessage) {
        if sender == self.myChannel {
                guard let channel = self.myChannel else { return }
                channel.markAsRead()
            if let delegate = self.delegate {
                if delegate.responds(to: #selector(GroupChannelsUpdateListDelegate.updateGroupChannelList)) {
                        delegate.updateGroupChannelList!()
            ...
            DispatchQueue.main.async {
                ...
                self.messages.append(message)
    //                self.tblChat.insertRows(at: [IndexPath(row: self.messages.count - 1, section: 0)], with: .none)
                    self.tblChat.reloadData()
                    self.tblChat.layoutIfNeeded()
                    self.scrollToBottom(force: false)
...
//    }
    ...
    @IBAction func sendMsgAction(_ sender: Any)
    {
        ...
        guard let messageText = self.msgTextView.text else { return }
        guard let channel = myChannel else {
            ...
            DispatchQueue.main.async {
                    ...
                    if let preSendMessage = self.preSendMessages[requestId] {
                        if let index = self.messages.firstIndex(of: preSendMessage) {
                            ...
                            self.preSendMessages.removeValue(forKey: requestId)
                            self.tblChat.reloadRows(at: [IndexPath(row: index, section: 0)], with: .none)
                            self.scrollToBottom(force: false)
                        }
                    }
                }
    ...
    private func sendImageFileMessage(imageData: Data, imageName: String, mimeType: String) {
        ...
        preSendMessage = channel.sendFileMessage(with: fileMessageParams, progressHandler: { [weak self] (bytesSent, totalBytesSent, totalBytesExpectedToSend) in
            DispatchQueue.main.async {
                ...
                guard let strongNewSelf = self else { return }
                
                if strongNewSelf.sendingImageVideoMessage[preSendMessageRequestId] == nil {
                    strongNewSelf.sendingImageVideoMessage[preSendMessageRequestId] = false
                }
                
                strongNewSelf.fileTransferProgress[preSendMessageRequestId] = CGFloat(totalBytesSent) / CGFloat(totalBytesExpectedToSend)
                for index in stride(from: strongNewSelf.messages.count - 1, to: -1, by: -1) {
                    let baseMessage = strongNewSelf.messages[index]
                    if baseMessage is SBDFileMessage {
                        let fileMessage = baseMessage as! SBDFileMessage
                        guard let fileMessageRequestId = fileMessage.requestId else { return }
                        if fileMessageRequestId == preSendMessageRequestId {
                            strongNewSelf.determineScrollLock()
                            let indexPath = IndexPath(row: index, section: 0)
                            if strongNewSelf.sendingImageVideoMessage[preSendMessageRequestId] == false {
                                strongNewSelf.tblChat.reloadRows(at: [indexPath], with: .none)
                                strongNewSelf.sendingImageVideoMessage[preSendMessageRequestId] = true
                                strongNewSelf.scrollToBottom(force: false)
                            }
                            else {
                                if let cell = strongNewSelf.tblChat.cellForRow(at: indexPath) as? GroupChannelOutgoingImageVideoFileMessageTableViewCell {
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime(uptimeNanoseconds: 150 * NSEC_PER_MSEC), execute: {
                    guard let fileMessageRequestId = fileMessage?.requestId else { return }
                    let preSendMessage = strongNewSelf.preSendMessages[fileMessageRequestId] as! SBDFileMessage
                    
                    strongNewSelf.preSendMessages.removeValue(forKey: fileMessageRequestId)
                    strongNewSelf.sendingImageVideoMessage.removeValue(forKey: fileMessageRequestId)
                    
                    if error != nil {
                        DispatchQueue.main.async {
                            strongNewSelf.resendableMessages[fileMessageRequestId] = preSendMessage
                            strongNewSelf.resendableFileData[preSendMessage.requestId!] = [
                                "data": imageData,
                                "type": mimeType,
                                "filename": imageName
                                ] as [String : AnyObject]
                            strongNewSelf.tblChat.reloadData()
                            strongNewSelf.scrollToBottom(force: true)
                        }
                    ...
                    if fileMessage != nil {
                        DispatchQueue.main.async {
                            strongNewSelf.determineScrollLock()
                            strongNewSelf.resendableMessages.removeValue(forKey: fileMessageRequestId)
                            strongNewSelf.resendableFileData.removeValue(forKey: fileMessageRequestId)
                            strongNewSelf.messages[strongNewSelf.messages.firstIndex(of: preSendMessage)!] = fileMessage!
                            let indexPath = IndexPath(row: strongNewSelf.messages.firstIndex(of: fileMessage!)!, section: 0)
                            strongNewSelf.tblChat.reloadRows(at: [indexPath], with: .none)
                            strongNewSelf.tblChat.layoutIfNeeded()
                            strongNewSelf.scrollToBottom(force: false)
                        }
                    }
                })
                        ...
                        if baseMessage is SBDFileMessage {
                            ...
                            if fileMessage.requestId != nil && fileMessage.requestId! == preSendMessage!.requestId! {
                                ...
                                if self.sendingImageVideoMessage[preSendMessage!.requestId!] == false {
                                    ...
                                    let cell = self.tblChat.cellForRow(at: indexPath) as! GroupChannelOutgoingImageVideoFileMessageTableViewCell
                                    cell.showProgress(CGFloat(totalBytesSent) / CGFloat(totalBytesExpectedToSend))
                                }
                        ...
                        if let message = fileMessage {
                            if let requestId = message.requestId {
                                DispatchQueue.main.async {
                                    ...
                                    self.tblChat.reloadRows(at: [fileMessageIndexPath], with: .none)
                                    self.scrollToBottom(force: false)
                                }
                            }
                        }
       ...
       func didLongClickGeneralFileMessage(_ message: SBDFileMessage) {
           ...
           if self.resendableFileData[requestId] == nil {
               ...
               let actionSave = UIAlertAction(title: "Save File", style: .default) { (action) in
                   DownloadManager.download(url: url, filename: message.name, mimeType: message.type, addToMediaLibrary: false) // 6 june
               }
               var actionDelete: UIAlertAction?
